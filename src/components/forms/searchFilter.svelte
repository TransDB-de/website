<script lang="ts">
	...
	const unsubscribe = navigating.subscribe((nav) => {
		if (
			nav &&
			nav.to.pathname === "/search" &&
			nav.from.pathname === "/search" &&
			Array.from(nav.to.searchParams).length < 1
		) {
			resetFilter();
		}
	});
	...
	<div class="filter" class:expand>
		{#if $currentLocation}
			<p class="sub-title">
				...
				<Button light class="reset-location" on:click={resetLocation}>
		...
		<Button class="mobile filter-button" on:click={applyFilters} title={t("mouseOverTexts.filter")}>
