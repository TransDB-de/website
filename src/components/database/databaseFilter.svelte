<script lang="ts">
	...
	let dropdownVisible = $derived(suggestions.length > 0 && focused);

	let dropdown: HTMLUListElement | undefined = $state();
	let suggestionElements: HTMLLIElement[] = $state([]);

	let filterElement: HTMLDivElement | undefined = $state();

	// Random Suggestion
	let randomSuggestion = $state("");
	$effect(() => {
...
<div class="text-filter" bind:this={filterElement}>
	...
	<Button onclick={filter}>Filtern</Button>
</div>
