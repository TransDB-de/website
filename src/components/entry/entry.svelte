<script lang="ts">
	...
	$: possibleDuplicateLink =
		entry.possibleDuplicate && $userdata?.admin
			? "/manage/database?id=" + entry.possibleDuplicate
			: "/entry/" + entry.possibleDuplicate;
...
<div class="entry">
	<div class="data">
		...
		<p class="small-gap">
			...
			{#if entry.firstName || entry.lastName}
				<span>
					{tEntry("academicTitleMapping")[entry.academicTitle] ?? ""}
					{entry.firstName ?? ""}
					{entry.lastName ?? ""}
				</span>
			{/if}
	...
	{#if !entry.blocked}
		<div class="controls">
			...
			{:else}
				<EdgeButton
					on:click={() => goto("/report?id=" + entry._id)}
