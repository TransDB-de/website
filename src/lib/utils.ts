...
import { errorMappings, type ErrorMappingContraints } from "./errorMappings";
...
export function parseValidationErrors(errors: ApiValidationError[]): ValidationErrorMap {
	...
	for (let error of errors) {
		let constraintName: ErrorMappingContraints | undefined = undefined;

		if (error.constraints) {
			constraintName = Object.keys(error.constraints)[0] as ErrorMappingContraints;
		...
		if (constraintName && constraintName in errorMappings) {
